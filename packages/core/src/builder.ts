...
import {
  AdurcBuildValidationError,
  AdurcModelDirectiveCompositionRestrictionValidationError,
  AdurcUnknownModelDirectiveBuildValidationError,
  AdurcUnknownModelSourceBuildValidationError,
  AdurcValidationContextError,
} from './errors/validation-context.error';
...
export class AdurcBuilder {
  ...
  public async build<T = Record<string, AdurcModelUntyped>>(): Promise<Adurc<T>> {
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      let register: BuilderGenerator | null = null;
      ...
      while ((register = registers.shift() ?? null)) {
  ...
  private validateContext() {
    ...
    for (const model of this._context.models) {
      ...
      for (const directive of model.directives) {
        const definition = this._context.directives.find(
          (x) => x.provider === directive.provider && x.name === directive.name
        );
