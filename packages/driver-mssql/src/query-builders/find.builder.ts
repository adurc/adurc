...
import {
  FindContextQueryBuilder,
  IAliasAccessor,
  IJoinQueryBuilder,
  IOrderableQueryBuilder,
  ITableAccessor,
} from './find.context';
...
export class FindQueryBuilder {
  ...
  public static buildTableAccessor(
    table: string,
    as: string,
    schema?: string,
    database?: string
  ): ITableAccessor & IAliasAccessor {
  ...
  public static buildOrderBy(
    orderBy: AdurcModelOrderBy<unknown>,
    entity: MSSQLEntity,
    context: IOrderableQueryBuilder
  ): void {
  ...
  public static buildInclude(
    include: AdurcModelInclude<AdurcModelUntyped>,
    entity: MSSQLEntity,
    context: FindContextQueryBuilder
  ): void {
  ...
  public static buildManyToMany(
    relation: MSSQLRelationManyToMany,
    context: FindContextQueryBuilder,
    value: AdurcFindManyArgs
  ): void {
    context.into = '#main';
    ...
    relationContext.from = this.buildTableAccessor(
      relation.manyEntity.tableName,
      'many',
      relation.manyEntity.schema,
      relation.manyEntity.database
    );
    ...
    relationContext.joins.push({
      type: 'inner',
      from: this.buildTableAccessor(
        relation.joinEntity.tableName,
        'root',
        relation.joinEntity.schema,
        relation.joinEntity.database
      ),
      conditions: [
        {
  ...
  public static buildOneToMany(
    relation: MSSQLRelationOneToMany,
    context: FindContextQueryBuilder,
    value: AdurcFindManyArgs
  ): void {
    context.into = '#main';
  ...
  public static buildManyToOne(
    relation: MSSQLRelationManyToOne,
    context: FindContextQueryBuilder,
    value: unknown
  ): void {
